...
{
    if (std::ifstream cmdlineFile("/proc/self/cmdline"); cmdlineFile.is_open())
    {
        std::string result;
        getline(cmdlineFile, result);
        if (size_t pos{ result.find_last_of('/') }; pos != std::string::npos)
        {
            std::string nullTerminatedProcName{ result.substr(pos + 1) };
            return nullTerminatedProcName.substr(0, nullTerminatedProcName.size() - 1);
        }
    }
...
inline void LogToStreamer(Level level, const char* fmt, va_list args)
{
    ...
    static const std::string processName{ GetProcessName() };
    const pid_t pid{ getpid() };
    const std::string processLogTag{ processName + " : " + std::to_string(pid) };
